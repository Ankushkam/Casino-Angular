...
export class HighlightDirective implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (!this.searchedWord || !this.searchedWord.length || !this.classToApply) {
      this.renderer.setProperty(this.el.nativeElement, 'innerHTML', this.content[this.field]);
      return;
    }
  ...
  getFormattedText() {
    ...
    return this.content[this.field].replace(re, `<span class="${this.classToApply}">$1</span>`);
  }
}